...
def ts_dag(result,
    ...
    for tau in range(TAU+1):
        nodes_tmp = [t + "_" + str(TAU - tau) for t in res.keys()]
        nodes.append(nodes_tmp)

    edges = list()
    edge_width = dict()
    for tau in reversed(range(0, TAU+1)):
        for t in res.keys():
            for s in res[t]:
                s_node = nodes[tau - s[LAG]][list(res.keys()).index(s[SOURCE])]
                t_node = nodes[tau][list(res.keys()).index(t)]
    ...
    Graph(edges, 
          ...
          node_layout_kwargs = dict(layers = nodes, reduce_edge_crossings = True),
          node_label_fontdict = dict(size = font_size),
          node_label = list(res.keys()),
          node_size = 10,
          node_color = node_color,
