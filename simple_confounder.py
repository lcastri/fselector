...
# init and run pcmci
var_name_copy = deepcopy(var_names_pretty)
var_name_copy.remove('$X_0$')
dataframe = pp.DataFrame(data[:,1:], var_names = var_name_copy)
...
tigraplot.plot_graph(
    ...
    graph=results['graph'],
    save_name='results/'+resfolder+'/wo_x0',
    var_names=var_name_copy,
...
tigraplot.plot_graph(
    val_matrix=results['val_matrix'],
    graph=results['graph'],
    save_name='results/'+resfolder+'/wo_selector_x0',
    var_names=var_name_copy,
    link_colorbar_label='cross-MCI',
    node_colorbar_label='auto-MCI',
    vmin_edges=0.,
